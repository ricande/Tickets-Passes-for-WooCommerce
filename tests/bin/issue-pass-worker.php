<?php
/**
 * Worker: issue parent passes for one order line under TPFW_Issue_Lock.
 *
 * Usage: php issue-pass-worker.php <product_id> <user_id> <order_id> <line_id> <qty> <prefix>
 */
if($argc < 7)
{
	fwrite(STDERR, "usage\n");
...
$aSync = TPFW_Issue_Lock::sync_line($wpdb, 'pass', $sTable, $sSelect, $iLineId, $iQty, $sNow, function() use ($wpdb, $sTable, $iProductId, $iUserId, $iOrderId, $iLineId, $sNow) {
	...
	$wpdb->query($wpdb->prepare(
		...
		array($sTable, $sNano, $iProductId, $iUserId, $iUserId, $iOrderId, $iLineId, 86400, 1, $sNow, $sNow)
	));
	return $sNano;
});
