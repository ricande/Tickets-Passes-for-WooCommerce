...
Prefix: `$wpdb->prefix` plus the name below. Schema: `inc/db-installer/class--db-installer.php` (`DB_VERSION` `1.0.5`). Rows are soft-deleted with `deleted` (timestamp); they are not removed.
