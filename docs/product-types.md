...
Issuing is always `woocommerce_order_status_completed` → `order_completed()` → `create_entry()` per matching line. Cancelling is `cancelled` / `refunded` / `failed` → `cancel_entry()` (soft-delete). Both are idempotent per order line.
...
`create_ticket()` is quantity-aware: existing rows for the same product/order/line are reinstated up to the current quantity (same `nano_id` / QR), surplus rows are soft-deleted, only the shortfall is inserted. A completed → refunded → completed cycle keeps the customer’s original codes.

## Timeslot Ticket (`tpfw-timeslot-ticket`)

Class: `TPFW_Timeslot_Ticket_WC_Product`. Issued rows: `tpfw_timeslot_tickets`. Capacity lives on `tpfw_timeslots`, not WooCommerce stock. The Inventory tab is hidden for this type.
...
1. Add to cart validates the chosen slot still has capacity and writes `tpfw_timeslot_reservations` (`valid_to` = hold expiry). Cart item meta carries `tpfw_timeslot_id` and `tpfw_reservation_id`.
...
5. Completed order issues `tpfw_timeslot_tickets` for the reserved slot. Cancelled/refunded/failed revokes the ticket and frees capacity.
...
**Guest passes** (`parent_nano_id_fk` set): quantity and duration come from `_tpfw_pass_guest_pass_*`. Their `valid_from` / `valid_to` stay empty until the **parent pass’s first successful check-in**, which runs `activate_guest_passes()` so a guest cannot walk in ahead of the holder.
