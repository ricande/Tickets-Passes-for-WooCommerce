...
Scheme and host are ignored so a ticket issued on `woocommerce.local` still checks in from a LAN IP. A random URL that is not this plugin’s check-in path is refused. The page then `GET`s `sCheckinBase + nano_id` on **its own origin** with the `wp_rest` nonce.
...
| Caller | How |
|---|---|
| Built-in scanner | Login cookie **and** valid `X-WP-Nonce`. Core’s `rest_cookie_check_errors()` clears the user if the nonce is missing, so a CSRF GET from another site cannot check anyone in. |
...
`GET /scanner/checkin/{nano_id}` probes, in order: `tpfw_pass`, `tpfw_timeslot_tickets`, `tpfw_tickets` (`TPFW_API::CHECKIN_TABLES`). Soft-deleted rows are ignored.
...
Wrapped in `with_checkin_lock()`: MySQL `GET_LOCK('tpfw_checkin_{nano_id}', 5)`. Contention answers **202**. `RELEASE_LOCK` runs in `finally` (persistent connections would otherwise wedge that code).
...
8. Success array (HTTP 200). Pass/guest include `sPhotoURL`
...
| 202 | Refused for a reason door staff must read (window, uses, cooldown, lock) |
| 401 | Does not add up (missing code, wrong type, insert failed, auth denied) |
